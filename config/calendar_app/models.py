...
def create_delivery_events_for_quote(order) -> CalendarEvent | None:
    ...
    if created:
        # Criar lembretes: 7 dias antes, 3 dias antes e no dia
...
def create_quote_followup_events(quote) -> list[CalendarEvent]:
    """
    Cria lembretes de follow-up para orçamentos em rascunho.
    - 3 dias após criação: primeiro follow-up
    - 7 dias após criação: segundo follow-up
    """
    from datetime import timedelta
    ...
    followup_config = [
        (3, "Follow-up: orçamento sem resposta (3 dias)"),
        (7, "Follow-up: orçamento ainda em rascunho (7 dias)"),
    ]

    for days_after, title in followup_config:
        followup_date = quote.quote_date + timedelta(days=days_after)
        ...
        if followup_date >= timezone.localdate():
            event, created = CalendarEvent.objects.get_or_create(
                ...
                defaults={
                    "title": title,
                    "description": f"Orçamento {quote.number} para {quote.customer.name} ainda em rascunho.",
                    "assigned_to": quote.seller,
                    "customer": quote.customer,
                },
            )
            if created:
                Reminder.objects.get_or_create(
                    event=event,
                    remind_date=followup_date,
                    defaults={
                        "message": f"{title} - {quote.customer.name}",
                    },
                )
            events.append(event)
